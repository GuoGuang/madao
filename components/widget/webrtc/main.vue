<template>
  <div id="global-webrtc">
    <div :class="aligenLeft ? 'aligenLeft' : ''" class="webrtc-box">
      <div
      ...
      >
        <div class="empty-msg">
          <!-- 本机用户 -->
          <span v-if="stream.local" class="text">
            <span v-if="localStream.ok">Long time no see.</span>
            <span v-else>Ice 失败</span>
          </span>
          <!-- 远程用户 -->
          <span v-else class="text">{{ stream.name }}</span>
        </div>
        <div :class="`filter-${filters[stream.filter]}`" class="content-box">
          <video
            :id="stream.id"
            :ref="stream.id"
            :src="stream.src"
        ...
        <div v-if="!stream.local" class="name">
        ...
        />
        <!-- 仅本机用户 -->
        <div v-if="stream.local" class="tools">
          <button
            :class="{ active: !localStream.disabledCarema }"
            class="video"
            @click="toggleCarema(!localStream.disabledCarema)"
          >
            <i :class="localStream.disabledCarema ? 'icon-carema-disabled' : 'icon-carema'" class="iconfont"/>
          </button>
          <button
          ...
          >
            <i :class="localStream.disabledMic ? 'icon-mic-disabled' : 'icon-mic'" class="iconfont"/>
          </button>
          <button
            ...
            @click="toggleBeauty()"
          >
            <i class="iconfont icon-meiyan"/>
            <span>美颜</span>
          </button>
          <div :class="{ active: stream.filter !== 0 }" class="filter">
            <span class="current">
              <i class="iconfont icon-filter"/>
              <span class="name">{{ filters[stream.filter] }}</span>
            </span>
            <ul class="filter-list">
              <li v-for="(filter, index) in filters" :key="index" class="item" @click="sendFilterToAll(index)">
                <span>{{ filter }}</span>
              </li>
            </ul>
          </div>
        ...
        <div v-else class="tools">
          <button :class="{ active: !stream.disabledCarema }" class="video" disabled>
            <i :class="stream.disabledCarema ? 'icon-carema-disabled' : 'icon-carema'" class="iconfont"/>
          </button>
          <button :class="{ active: !stream.disabledMic }" class="audio" disabled>
            <i :class="stream.disabledMic ? 'icon-mic-disabled' : 'icon-mic'" class="iconfont"/>
          </button>
          <button :class="{ active: stream.beauty }" class="beauty" disabled>
            <i class="iconfont icon-meiyan"/>
          </button>
          <span class="filter">
            <span class="current">
